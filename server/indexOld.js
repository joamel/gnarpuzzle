...
app.delete('/participants/:roomId/:username', (req, res) => {
  ...
  if (participants[roomId]) {
    ...
    // Check if room is empty and clear chat if so
    if (participants[roomId].length === 0) {
      console.log(`Room ${roomId} is now empty, clearing chat history`);
      chatMessages[roomId] = [];
      // Emit chat clear event to any remaining connections
...
io.on("connection", (socket) => {
	...
	socket.on("leaveRoom", ({ room, username }) => {
		...
		if (user) {
			...
			// Check if room is now completely empty
			...
			if (remainingSocketUsers.length === 0 && remainingParticipants.length === 0) {
				...
				chatMessages[room] = [];
				// Clean up room game state as well
				if (roomGameStates[room]) {
					delete roomGameStates[room];
				}
			}
		}
	});
